...
class Encoder(nn.Module):
    def __init__(self, input_size, hidden_size, bidirectional=True):
        ...
        self.rnn = nn.RNN(input_size, self.hidden_size, num_layers=1, bidirectional=bidirectional, batch_first=True)
    ...
    def forward(self, x, x_len):
        ...
        if self.bidirectional:
            ...
            state = state.transpose(0, 1).reshape(state.size(1), -1)
        return output, state
...
class Seq2seq(nn.Module):
    ...
    def forward(self, x, x_len):
        ...
        #######################################################################
        # Encoder
        encoder_outputs, encoder_state = self.encoder(x, x_len)
        
        #######################################################################
        # Decoder
        decoder_input = self.embedding(torch.LongTensor(np.random.choice(self.vocab_size, batch_size)).to(device)) # (batch_size, 1, embed_size)
